...
public class RUHungry {
    ...
    public void menu(String inputFile) {
        ...
        for (int i = 0; i < numCategories; i++) {
            ...
            for (int j = 0; j < numDishes; j++) {
                ...
                newNode.setNextMenuNode(menuVar[i]);
                menuVar[i] = newNode;
            }
        }
    ...
    public void updateStock (String ingredientName, int ingredientID, int stockAmountToAdd) {
        ...
        if (ingredientName != null) {
            targetNode = findStockNode(ingredientName);
        } 
        else if (ingredientID != -1) {
            targetNode = findStockNode(ingredientID);
        } 
        else{
            targetNode=null;
        }
        ...
        if (targetNode != null) {
            ...
            int currentStock = targetNode.getIngredient().getStockLevel();
            int newStock = currentStock + stockAmountToAdd;
            targetNode.getIngredient().setStockLevel(newStock);
        }
    ...
    public void updatePriceAndProfit() {
        for (int i = 0; i < menuVar.length; i++) {
            MenuNode currentCategory = menuVar[i];
            
            // Iterate over each dish in the current category
            while (currentCategory != null) {
                Dish currentDish = currentCategory.getDish();
                double dishCost = 0.0;
    
                // Iterate over each ingredient in the current dish
                for (int ingredientID : currentDish.getStockID()) {
                    // Find the StockNode for the current ingredient
                    StockNode ingredientNode = findStockNode(ingredientID);
                    if (ingredientNode != null) {
                        // Add the cost of the ingredient to the dish cost
                        dishCost += ingredientNode.getIngredient().getCost();
                    }
                }
    
                // Compute dish price and profit
                double dishPrice = dishCost * 1.2;
                double profit = dishPrice - dishCost;
    
                // Update the price and profit of the dish
                currentDish.setPriceOfDish(dishPrice);
                currentDish.setProfit(profit);
    
                // Move to the next dish in the current category
                currentCategory = currentCategory.getNextMenuNode();
            }
        }
    ...
    public void createStockHashTable (String inputFile){
        ...
        while (!StdIn.isEmpty()) {
            ...
            // Read ingredient name (remove leading space)
            StdIn.readChar(); // Consume space character
            String ingredientName = StdIn.readLine().trim();
            
            // Read ingredient cost
            ...
            // Create StockNode referring to the Ingredient object
            StockNode newNode = new StockNode(ingredient, null);
            
            // Add the StockNode to the stockVar hashtable
    ...
    public void order (String dishName, int quantity){
        ...
        if (checkDishAvailability(dishName, quantity)) {
            // If the dish can be prepared, create a TransactionData object
            MenuNode menuNode = findDish(dishName);
            double dishProfit = menuNode.getDish().getProfit() * quantity;
            TransactionData orderTransaction = new TransactionData("order", dishName, quantity, dishProfit, true);
            addTransactionNode(orderTransaction);
    
            // Update stock for each ingredient of the dish
            Dish dish = menuNode.getDish();
            int[] stockIDs = dish.getStockID();
            for (int stockID : stockIDs) {
                updateStock(null, stockID, -quantity);
            }
            //return;
        } else {
            // If the dish cannot be prepared, record the transaction as unsuccessful
            TransactionData orderTransaction = new TransactionData("order", dishName, quantity, 0, false);
            addTransactionNode(orderTransaction);
    
            // Simulate trying to order other dishes in the same category
            MenuNode currentDishNode = findDish(dishName);
            MenuNode origDishNode = findDish(dishName);
            while (true) {
                currentDishNode = currentDishNode.getNextMenuNode();
                if (currentDishNode == null) {
                    // If reached end of category, start from the beginning
                    currentDishNode = menuVar[findCategoryIndex(findDish(dishName).getDish().getCategory())];
                }
                if (currentDishNode == origDishNode) {
                    // If checked all dishes in the category
                    //TransactionData orderTransaction1 = new TransactionData("order", dishName, quantity, 0, false);
                    //addTransactionNode(orderTransaction1);
                    break;
                }
                dishName = currentDishNode.getDish().getName();
                if (checkDishAvailability(dishName, quantity)) {
                    // If the next dish can be prepared, order it and return
                    order(dishName, quantity);
                    return;
                }else{
                    TransactionData orderTransaction2 = new TransactionData("order", dishName, quantity, 0, false);
    ...
    public double profit () {
        ...
        while (currentTransaction != null) {
            if (currentTransaction.getData().getProfit()!=0) {
                totalProfit += currentTransaction.getData().getProfit();
            }
	...
	// WRITE YOUR CODE HERE
        if (profit() > 50.0) {
            ...
            if (stockNode != null && stockNode.getIngredient().getStockLevel() >= quantity) {
                // Successful donation
                double profit = 0; 
                TransactionData donationTransaction = new TransactionData("donation", ingredientName, quantity, profit, true);
                addTransactionNode(donationTransaction);
                updateStock(ingredientName, -1, -1 * quantity);
            } else {
                // Failed donation due to insufficient stock
                TransactionData donationTransaction = new TransactionData("donation", ingredientName, quantity, 0, false);
                addTransactionNode(donationTransaction);
            }
        } else {
            // Failed donation due to insufficient profit
            TransactionData donationTransaction = new TransactionData("donation", ingredientName, quantity, 0, false);
            addTransactionNode(donationTransaction);
        }
	...
	// WRITE YOUR CODE HERE
        ...
        StockNode stockNode = findStockNode(ingredientName);
        if (stockNode != null) {
            cost = stockNode.getIngredient().getCost() * quantity;
        }

        // Check if there's enough total profit in the day to pay for the restock request
        if (profit() >= Math.abs(cost)) {
            // Successful restock
            TransactionData restockTransaction;
            restockTransaction = new TransactionData("restock", ingredientName, quantity, -cost, true);
            // Update stock accordingly
            updateStock(ingredientName, -1, quantity);
            
            addTransactionNode(restockTransaction);
        } else {
            // Failed restock due to insufficient profit
